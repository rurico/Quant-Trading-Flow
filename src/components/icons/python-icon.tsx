...
import React from 'react'; // 移除了 useEffect, useState
...
export function PythonIcon({ iconClassName, gradientIdSuffix, ...props }: PythonIconProps) {
  // 确保 gradientIdSuffix 是干净的。理想情况下，这应该由调用者完成。
  // 然而，为了安全起见，我们可以在这里清理它。如果它已经是干净的，这不会改变它。
  // 允许下划线和连字符，因为它们是有效的ID字符。
  const cleanedSuffix = gradientIdSuffix.replace(/[^a-zA-Z0-9_-]/g, '_'); 

  const paint0Id = `paint0_linear_python_${cleanedSuffix}`;
  const paint1Id = `paint1_linear_python_${cleanedSuffix}`;
